...
public class EGA_Laser : MonoBehaviour
{
    ...
    void Start ()
    {
        layerMask = ~LayerMask.GetMask("Unshootable");
    ...
    void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(new Vector3(transform.position.x - 0.12f, transform.position.y, transform.position.z), transform.TransformDirection(Vector3.forward), out hit, MaxLength, layerMask))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
